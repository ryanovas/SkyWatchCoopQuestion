...
Algorithm:
1. Attempt to split the query by with keyword
2. Determine if the query has both object types and physical criteria or not.
3. For each part of the query, further split the string into tokens delimited by ',' or 'and'.
4. For physical criteria, convert comparator symbols to the correct comparator strings (a la mongodb docs) while only removing whitespace between the comparator and operands.
